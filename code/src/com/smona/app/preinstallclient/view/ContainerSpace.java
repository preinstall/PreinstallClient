...
@SuppressLint("NewApi")
public class ContainerSpace extends FrameLayout implements DragSource, DropTarget, DragListener {
	...
	private void initScrollGridView() {
		...
		for (int i = 0; i < pageNo; i++) {
			...
			MainDataAdatper mDataAdapter = new MainDataAdatper(mMain, loaclDatasource);
	...
	public void onDropCompleted(View target, DragObject dragObject, boolean isFlingToDelete, boolean success) {
		LogUtil.d(TAG, "onDropCompleted dragObject=" + dragObject + ", isFlingToDelete: " + isFlingToDelete + ", success: " + success);
		if (success) {
			if (target != this) {
	...
	public void onDrop(DragObject d) {
		LogUtil.d(TAG, "onDrop dragObject=" + d + ", d.dragView.hasDrawn(): " + d.dragView.hasDrawn());
		if (d.dragView.hasDrawn()) {
			mMain.getDragLayer().animateViewIntoPosition(d.dragView, mDragInfo.cell);
		} else {
	...
	private void beginDragShared(View view, DragSource source) {
		...
		float scale = mMain.getDragLayer().getLocationInDragLayer(view, mTempXY);
		int dragLayerX = Math.round(mTempXY[0] - (bmpWidth - scale * view.getWidth()) / 2);
		int dragLayerY = Math.round(mTempXY[1] - (bmpHeight - scale * bmpHeight) / 2);
		LogUtil.d(TAG, "view.getTag(): " + view.getTag());
		mDragController.startDrag(mDragOutline, dragLayerX, dragLayerY, source, mDragInfo, DragController.DRAG_ACTION_MOVE, null, null, scale);
	}
	...
	private Bitmap createDragOutline(View v, Canvas canvas, DragSource source) {
		...
		final Bitmap b = Bitmap.createBitmap(imgW, imgH, Bitmap.Config.ARGB_8888);
	...
	static void drawDragView(Context context, View v, int imgW, int imgH, Canvas destCanvas, Rect rect) {
	...
	private Animator createImageAnimator(final View view) {
		ClientViewPropertyAnimator imageAnim = new ClientViewPropertyAnimator(view);
		imageAnim.alpha(0f).translationX(0).translationY(-ANIMATION_DELETE_TRANSLATION_Y).setDuration(ANIMATION_DELETE_DURATION);
	...
	private Animator createTitleAnimator(View view, final RemoveCallback callback) {
		ClientViewPropertyAnimator imageAnim = new ClientViewPropertyAnimator(view);
		imageAnim.alpha(0f).translationX(0).translationY(ANIMATION_DELETE_TRANSLATION_Y).setDuration(ANIMATION_DELETE_DURATION);
		imageAnim.addListener(new AnimatorListenerAdapter() {
			public void onAnimationStart(Animator animation) {
