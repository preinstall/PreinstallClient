...
public class ProcessModel extends BroadcastReceiver {
	...
	private static final HandlerThread WORKER_THREAD = new HandlerThread("loader");
	static {
		WORKER_THREAD.start();
	}
	...
	public static void saveToDB(ClientApplication mApp, List<ItemInfo> datas) {
		...
		for (int i = 0; i < datas.size(); i++) {
			...
			c = contentResolver.query(ClientSettings.ItemColumns.CONTENT_URI, null, ClientSettings.ItemColumns.PACKAGENAME + " = '" + info.packageName + "'", null, null);
	...
	public static void requestIcons(ClientApplication mApp, IconCache mIconCache, List<ItemInfo> datas) {
		int size = datas.size();
		ItemInfo info = null;
	...
	public static void requestIcon(ClientApplication mApp, IconCache mIconCache, final ItemInfo info) {
		ImageCacheStrategy.ReturnImageType type = ImageCacheStrategy.getInstance().downloadImage(info.packageName, info.appIconUrl);
		LogUtil.d(TAG, "requestIcon type=" + type + ", info.appIconUrl: " + info.appIconUrl);
	...
	public static void filterDulicateMemory(ClientApplication mApp, List<ItemInfo> datas) {
		PackageManager manager = mApp.getPackageManager();
		Intent intent = new Intent(Intent.ACTION_MAIN);
		intent.addCategory(Intent.CATEGORY_LAUNCHER);
		List<ResolveInfo> memoryDatas = manager.queryIntentActivities(intent, 0);
	...
	public static void filterDulicateDB(ClientApplication mApp, List<ItemInfo> datas) {
	...
	public static void insertDB(ContentResolver contentResolver, ItemInfo info) {
		...
		values.put(ClientSettings.ItemColumns.DOWNLOADSTATUS, info.downloadStatus);
	...
	public static void updatetDB(ContentResolver contentResolver, ItemInfo info) {
		...
		values.put(ClientSettings.ItemColumns.APPSIZE, info.appSize);
		values.put(ClientSettings.ItemColumns.SDKVERSION, info.sdkVersion);
		String where = ClientSettings.ItemColumns.PACKAGENAME + "='" + info.packageName + "'";
		int result = contentResolver.update(ClientSettings.ItemColumns.CONTENT_URI_NO_NOTIFICATION, values, where, null);
	}

	public static int updateDB(Context context, String packageName, ContentValues values) {
		ContentResolver resolver = context.getContentResolver();
		String where = ClientSettings.ItemColumns.PACKAGENAME + "='" + packageName + "'";
		int result = resolver.update(ClientSettings.ItemColumns.CONTENT_URI_NO_NOTIFICATION, values, where, null);
		return result;
	}
	...
	public static int deleteDB(Context context, String packageName) {
		...
		int result = resolver.delete(ClientSettings.ItemColumns.CONTENT_URI_NO_NOTIFICATION, where, new String[] { packageName });
		return result;
	}
