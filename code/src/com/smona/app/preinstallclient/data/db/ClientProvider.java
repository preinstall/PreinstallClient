...
public class ClientProvider extends ContentProvider {
	...
	public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs, String sortOrder) {
		...
		Cursor result = qb.query(db, projection, args.mWhere, args.mArgs, null, null, sortOrder);
	...
	public Uri insert(Uri uri, ContentValues values) {
		...
		if (rowId <= 0) {
			LogUtil.d(TAG, "insert() uri=" + uri + ", values: " + values + ", rowId: " + rowId);
			return null;
		}
	...
	public int delete(Uri uri, String where, String[] whereArgs) {
		SQLiteDatabase db = mOpenHelper.getWritableDatabase();
		int count = db.delete(ClientSettings.ItemColumns.TABLE_NAME, where, whereArgs);
		if (count > 0) {
			sendNotify(uri);
		}
		return count;
